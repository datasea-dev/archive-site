...
// Pastikan import ini mengarah ke folder yang benar sesuai struktur folder Anda
...
export default function Home() {
  ...
  // State untuk menangani PDF Viewer
  ...
  const handleResultClick = (file: SearchResult) => {
    ...
    const isPdfTitle = file.title.toLowerCase().includes(".pdf");

    if (isPdfType || isPdfTitle) {
       console.log("✅ Terdeteksi sebagai PDF! Membuka Viewer...");
       const proxyUrl = `/api/pdf?id=${file.id}`;
       setSelectedPdf({ url: proxyUrl, title: file.title });
       setIsFocused(false);
    } else {
       console.log("❌ Bukan PDF. Membuka Link Eksternal...");
  ...
  return (
    <>
      {/* MODE 1: PDF VIEWER ACTIVE 
        Render HANYA ini jika ada file dipilih. 
        z-[99999] memastikan dia menutupi Navbar dari Layout.
      */}
      {selectedPdf && (
      ...
      {/* MODE 2: NORMAL PAGE CONTENT 
        Render HANYA jika TIDAK ada file dipilih (!selectedPdf).
        Ini akan menyembunyikan Hero, Search, dan Grid di belakang saat PDF terbuka.
      */}
      {!selectedPdf && (
